...
export class NewsDetail implements OnInit {
  ...
  // Mock data - sẽ được thay thế bằng service
  newsList: News[] = [
    {
      id: 'news-001',
...
**Những ngân hàng dẫn đầu về lãi suất ưu đãi:**

1. **Vietcombank**: Lãi suất cho vay kinh doanh từ 5.2%/năm
2. **BIDV**: Chương trình ưu đãi SME với lãi suất từ 5.5%/năm  
3. **VietinBank**: Gói vay cá nhân với lãi suất từ 6.8%/năm
4. **Techcombank**: Lãi suất thả nổi từ 5.8%/năm

**Nguyên nhân giảm lãi suất:**

- Chính sách tiền tệ nới lỏng của Ngân hàng Nhà nước
- Cạnh tranh gay gắt giữa các ngân hàng
- Nhu cầu vay vốn của doanh nghiệp tăng cao
- Thanh khoản dồi dào trên thị trường

**Lợi ích cho khách hàng:**

Việc giảm lãi suất mang lại nhiều lợi ích thiết thực:
- Giảm gánh nặng tài chính cho người vay
- Tăng khả năng tiếp cận vốn của doanh nghiệp
- Thúc đẩy hoạt động đầu tư và tiêu dùng
- Hỗ trợ phục hồi kinh tế sau đại dịch

Dự báo trong những tháng cuối năm, lãi suất sẽ tiếp tục được điều chỉnh theo hướng tích cực để thúc đẩy tăng trưởng kinh tế. Các chuyên gia khuyến nghị khách hàng nên tận dụng thời điểm này để vay vốn phục vụ nhu cầu kinh doanh và tiêu dùng.`,
...
Hãy luôn đọc kỹ hợp đồng và hỏi rõ tất cả các khoản phí trước khi ký.`,
  ...
  private loadNews(id: string) {
    this.news = this.newsList.find(news => news.id === id) || null;
  ...
  private loadRelatedNews(currentId: string) {
    if (this.news) {
      // Lấy tin tức cùng danh mục (trừ tin hiện tại)
      this.relatedNews = this.newsList
        .filter(news => 
          news.id !== currentId && 
          news.danhMuc === this.news!.danhMuc
        )
        .slice(0, 3);
    }
  }
  ...
  shareNews() {
    if (navigator.share && this.news) {
      navigator.share({
        title: this.news.tieuDe,
        text: this.news.tomTat,
